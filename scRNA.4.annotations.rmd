...
# We Need to Compute Clusters Again. Didn't save in scRNA.3.marker_genes.r
...
p1 + p2
ggsave("6.annotated_plot.png", width = 400, height = 150, units = "mm")

p3 <- DimPlot(combinedSeurat,
  reduction = "umap",
...
p3
ggsave("6.seurat_cluster_plot.png", width = 400, height = 150, units = "mm")
```

Remove T cytotoxic cells to see cleaner dimplots:
```{r}
hcc_only <- subset(combinedSeurat, orig.ident == "HCC" & cell_type != "T cytotoxic cells")
hccf_only <- subset(combinedSeurat, orig.ident == "HCCF" & cell_type != "T cytotoxic cells")
p1 <- DimPlot(hcc_only,
  reduction = "umap",
  group.by = "cell_type",
  label = TRUE,
...
p1 + p2
ggsave("6.subsetted_annotated_plot.png", width = 400, height = 150, units = "mm")
```

Note that maybe the actual annotations are not super crucial. We just need to find similar cells 
between scRNAseq and scATACseq for comparison. 

Use the same annotation files for both????